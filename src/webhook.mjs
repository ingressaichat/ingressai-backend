...
ticketsRouter.get("/tickets/pdf", async (req, res) => {
  ...
  const BG = "#F5F7FA", CARD = "#FFFFFF", OUTLINE = "#E5E7EB";
  const TXT = "#111111", META = "#6E6E73", META_LIGHT = "#8E8E93";
  const ACCENT = "#007AFF", DIVIDER = "#C7C7CC";

  const BASE = 12;
  const scale = (n) => Math.round(BASE * Math.pow(PHI, n));
  const f_title = scale(1.9), f_meta = scale(0.2), f_name = scale(1.4), f_venue = 12, f_foot = scale(-0.3);
  ...
  doc.save().lineWidth(0.8).strokeColor(OUTLINE).roundedRect(card.x, card.y, card.w, card.h, card.r).stroke().restore();

  let y = card.y + pad;

  const logo = await fetchImageBuffer(LOGO_URL);
  if (logo) {
    const lw = 90, logoX = card.x + (card.w - lw) / 2;
  ...
  const qrBuf = await QRCode.toBuffer(validateUrl, { errorCorrectionLevel: "H", margin: 1, color: { dark: "#000000", light: "#FFFFFFFF" }});
...
/** Compra direta (gera e envia PDF por WhatsApp) */
  ...
  try {
    ...
    await axios.post(
      `https://graph.facebook.com/v20.0/${process.env.PHONE_NUMBER_ID}/messages`,
      {
        ...
        to: chatTo,
        type: "document",
        document: { link: pdfUrl, filename, caption: `✅ Compra confirmada!\n${pureEventName(ev)} • ${ev.city}\nQuantidade: ${qtyN}` }
      },
      {
...
/** Emite ticket de um orderId (envia PDF no WhatsApp) */
  ...
  try {
    ...
    await axios.post(
      `https://graph.facebook.com/v20.0/${process.env.PHONE_NUMBER_ID}/messages`,
      {
        ...
        to: data.to,
        type: "document",
        document: { link: pdfUrl, filename, caption: `✅ Ingresso emitido!\n${pureEventName(ev)} • ${ev.city}\nQuantidade: ${data.qty}` }
      },
      {
        headers: { "Content-Type": "application/json" },
        params: { access_token: TOKEN, ...(APP_PROOF ? { appsecret_proof: APP_PROOF } : {}) }
